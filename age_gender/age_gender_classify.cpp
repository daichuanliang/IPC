...
Tensor ReadTensorFromCvMat(cv::Mat image, const int height, const int width, const int depth) {

	Tensor input_tensor(tensorflow::DT_FLOAT, tensorflow::TensorShape({ 1, height, width, depth })); // New Tensor shape [1, ndim]
...
void ClassifyUsingCvMat(cv::Mat image_mat, string graph, std::vector<string> labels, std::unique_ptr<tensorflow::Session>* session) {
	// These are the command-line flags the program can understand.
	// They define where the graph and input data is located, and what kind of
	// input the model expects. If you train your own model, or use something
	// other than inception_v3, then you'll need to update these.
	...
	const Tensor image_tensor = ReadTensorFromCvMat(image_mat, input_height, input_width, 3);
...
void ClassifyAgeUsingCvMat(cv::Mat image_mat, std::unique_ptr<tensorflow::Session>* session) {
	LOG(INFO) << "Guess age...";
	ClassifyUsingCvMat(image_mat, "age_gender/model/age_inception_model.pb", AGE_LIST, session);
}

void ClassifyGenderUsingCvMat(cv::Mat image_mat, std::unique_ptr<tensorflow::Session>* session) {
	LOG(INFO) << "Guess gender...";
	ClassifyUsingCvMat(image_mat, "age_gender/model/gender_inception_model.pb", GENDER_LIST, session);
}
